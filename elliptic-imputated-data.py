...
'''
# 读取数据集
df = pd.read_csv('completed_data_1.csv')
# 将数据分为特征和标签
X = df.iloc[:, 2:]  # 假设第1列是ID，第2列是分类标签
y = df.iloc[:, 1]  # 分类标签
'''

# 读取数据集
df = pd.read_csv('deleted_data.csv')
# 将数据分为特征和标签
X = df.iloc[:, 1:-1] 
y = df.iloc[:, -1]  
...
def calculate_iba(sensitivity,specificity , alpha=0.5):
    ...
    g_mean = (sensitivity * specificity ) ** 0.5  # G-Mean 计算
    iba = (1 + alpha * dominance) * g_mean ** 2  # 根据公式计算 IBA
    return iba
